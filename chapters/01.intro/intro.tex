...
The interferometer only samples as many Visibilities as it has antenna pairs. The task is to reconstruct the observed image from an under-sampled Fourier space. In Astronomy, the CLEAN class of algorithms\cite{hogbom1974aperture}\cite{schwab1984relaxing}\cite{rich2008multi}\cite{rau2011multi} were developed for this task. It uses a fixed prior to reconstruct the image and has produced remarkable results in the past.

New interferometers like MeerKAT observe new phenomenons. To improve the image reconstruction, one would want to modify and add to the prior of CLEAN as  and model
...
The ideal interferometer would fully sample the Fourier Space and introduce no instrumental effects. The observed image can then be retrieved by the inverse Fourier Transform. A real world interferometer corrupts the observed image with the image with under-sampling, the antenna beam-pattern and other instrumental effects. The inverse Fourier Transform of the Visibilities can still be calculated, but it results in a corrupted, "dirty" image. The real world interferometer convolves the true image with a Point Spread Function(PSF), which results in the "dirty" image. The task is to deconvolve the "dirty" image with the PSF and retrieve the true image. More formally, we try to find a solution $x$ for equation \eqref{intro:eq:deconvolve}, where only the $PSF$ and $I_{dirty}$ are known.

\begin{equation}\label{intro:eq:deconvolve}
x \star  PSF + N = I_{dirty} 
\end{equation}

PSF models the instrumental effects. A different instrument olny changes the PSF\footnote{This is of course only true for small field of view imaging. Wide field of view imaging introduces more complex instrumental effects.} can be estimated

CASA produces a the $I_{dirty}$ image and the PSF for the VLA instrument.
