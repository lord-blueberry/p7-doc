...
With Compressed Sensing the regularization scheme is a separate part of the overall approach. It consists of three separate parts:
\begin{itemize}
...
CLEAN can be converted into a Compressed Sensing approach. The CLEAN objective consists only of a data term. By adding a regularization term, we arrive at the new Compressed Sensing CLEAN objective \eqref{intro:eq:csclean}. The data term forces the reconstruction to be close to the measurement, while the regularization term forces the reconstruction to be plausible. $\lambda$ models the expected noise in the problem. More noise needs more regularization, which needs a larger $\lambda$. Note that the zero norm $\left \| Px \right \|_0$ acts as an indicator function. 

\begin{equation}\label{intro:eq:csclean}
	\underset{x}{minimize} \: \left \| D_{dirty} - x \star PSF \right \|_2^2 \: + \: \lambda \left \| Px \right \|_0
\end{equation}

The next step is choosing a Prior $P$ that models the same regularization scheme. CLEAN assumes the $x$ contains a few point sources. In Compressed Sensing terminology, it assumes $x$ is sparse in image space. Since $x$ is already an image, the Prior $P$ in Compressed Sensing CLEAN is the identity matrix.

The last conversion step is choosing a similar optimization algorithm. In every iteration, CLEAN searches the highest peak in the dirty image. It is a greedy optimization scheme that is similar to Matching Pursuit.
