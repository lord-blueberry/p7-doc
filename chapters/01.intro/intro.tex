...
In nature, we encounter continuous signals: Music, pictures,  Computers can only measure discrete samples. How many samples are needed to capture a signal?

We want to convert a continuous signal into discrete samples: A music piece, an image of a tree. How many samples are needed to guarantee we know everything about the signal. Nyquist-Shannon Sampling Theorem: The sampling rate needs to be more than twice the highest frequency of the signal. Recording music, we the highest tone of the piece is 20kHz, our sampling rate should be more than 40kHz. Then our measurements guarantee to capture the continuous time signal.

Unique signal

However the Nyquist-Shannon Sampling Rate is not always achievable. Fully sampling is expensive, samples get corrupted, or the instrument itself. In these cases we deal with under-sampled measurements: Many possible signals fit the measurements, we want to find the true signal from.

The Theory of Compressed Sensing is a recent Sampling Theorem. We can find the true signal from under-sampled measurements by using our prior knowledge about the signal. Different Applications like compression, de-noising and in-painting.

In this project, the Theory of Compressed Sensing\cite{candes2006robust}\cite{donoho2006compressed} is applied to Image Reconstruction from under-sampled measurements. in the field of radio interferometers.


\subsection{Image Reconstruction for Radio Interferometers}
Measure arrival time differences of photons.

In small field of view, it each antenna pair measures approximately a Fourier Component. Phase and Amplitude. measurements. 
...
CLEAN class of algorithms reconstructs the image from under-sampled measurements.

The CLEAN class of algorithms\cite{hogbom1974aperture}\cite{schwab1984relaxing}\cite{rich2008multi}\cite{rau2011multi} are used to reconstruct the image and is the de-factor standard. Fixed prior assumption about the image.
...
%Interferometers do not observe the sky directly. The observed image has to be reconstructed from the measurements. The reconstruction is an ill-posed inverse problem. 
