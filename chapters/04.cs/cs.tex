...
The question remains, how many samples are needed to reconstruct the image? \cite{candes2011probabilistic}
If the image also contains extended emissions, it increases the number of non-zero pixels $s$. In turn, this may increase the amount of information we need, which may increase the number of samples needed. However, if we can find a domain in which images with extended emissions and point sources can be sparsely represented, we can reduce the needed samples.
